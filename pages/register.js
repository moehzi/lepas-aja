...
const register = () => {
  ...
  return (
    <section className="w-full min-h-[80vh] flex flex-col justify-center items-center py-8">
      <h1 className="text-5xl text-orange mb-14">Sign Up</h1>
      <form className="mb-4 flex flex-col max-w-[450px] w-full" onSubmit={handleRegister}>
        {error && (
        ...
        <div className="mb-4 shadow appearance-none border-red border rounded py-2 px-4 text-gray-700 leading-tight flex flex-col bg-white">
          ...
          <input onChange={(e) => setEmail(e.target.value)} className="bg-white focus:outline-none focus:shadow-outline text-red text-xl" placeholder="Enter your email/username here" />
        </div>
        <div className="mb-4 shadow appearance-none border-red border rounded py-2 px-4 text-gray-700 leading-tight flex flex-col bg-white">
          <label className="text-orange mb-1 border-b-[3px] border-red max-w-max" htmlFor="password">
            Password
          </label>
          <input onChange={(e) => setPassword(e.target.value)} className="bg-white focus:outline-none focus:shadow-outline text-red text-xl" id="password" type="password" placeholder="Enter your password here" />
        </div>
        <div className="flex flex-col items-center gap-4">
          ...
          <Link href="/login">
            <button className="text-lg font-bold text-black opacity-50">Already A Member?</button>
          </Link>
        </div>
