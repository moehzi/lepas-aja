...
const HistoryRoom = () => {
  ...
  return (
    <div className="min-h-[80vh] container mx-auto text-lg text-red max-w-[1050px] rounded-[10px] border border-[#C4C4C4] my-4 p-4">
      ...
      <div className="max-w-[950px] mt-8">
        {currentHistory.map((item, index) => {
          return <CardHistory date={item.date} roomDesc={item.roomDesc} src={item.image} owner={item.owner} status={item.status} title={item.title} />;
        })}
        <Pagination historyPerPage={historyPage} totalHistory={history.length} paginate={paginate} />
      </div>
    </div>
  );
};
